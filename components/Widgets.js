   ...
   return (
      <div className="sticky top-0 hidden lg:inline max-w-[425px] min-w-[350px] py-1 space-y-2.5 pl-8 h-max mb-[100px]">
         <div className={`sticky top-0 py-1.5 bg-black z-50 w-11/12 xl:w-10/12`}>
            <div
               className={`flex items-center ${searchFocus ? 'bg-transparent border border-blue-600' : 'bg-zinc-800/80'} py-3 px-5 rounded-full relative`}
               onClick={() => searchRef.current.focus()}
            >
               <RiSearchLine className={`text-xl ${searchFocus ? 'text-blue-600' : 'text-gray-500'}`} />
               <input
                  type="text"
                  ref={searchRef}
         ...
         <p className="pl-3 text-[12.5px] font-[500] text-gray-500">&copy; 2022 Twitter, Inc.</p>
